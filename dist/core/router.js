...
class LlmRouter {
    ...
    async decide(rc) {
        ...
        const draftOpen = (rc.ctx.state.emailDraft != null &&
        ...
        const lastEventExists = rc.ctx.state.lastEvent != null &&
            typeof rc.ctx.state.lastEvent === "object";
        const text = rc.input.kind === "text" ? rc.input.text.toLowerCase() : "";
        const looksLikeEventEdit = lastEventExists &&
            /\b(also|too|as well|invite|add|remove|change|move|reschedule|rename|update|push it|shift|it|that meeting|this meeting|the meeting|the event)\b/.test(text);
        if ((draftOpen || looksLikeEventEdit) &&
