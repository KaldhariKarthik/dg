"use strict";
/**
 * src/store/firestoreStores.ts — Firestore implementors (prod backing).
 *
 * Implements every persistence seam against Firestore:
 *   - FirestoreUserStore     -> users/{userId}
 *   - FirestoreSessionStore  -> sessions/{sessionId}
 *   - FirestoreWorkingStore  -> working/{userId}      (the planner/vision bag)
 *   - FirestoreMemoryStore   -> memory/{userId}
 *
 * Concurrency: the one place a lost update would corrupt identity —
 * mergeGoogleTokens preserving a refresh_token — uses a transaction. The
 * working-state bag keeps load/save (full-replace) semantics for now to match
 * existing behavior; atomic per-field plan updates come in the planner step.
 */
...
class FirestoreUserStore {
    ...
    async setIntegration(userId, app, cred) {
        await this.col
            .doc(userId)
            .set({ integrations: { [app]: cred } }, { merge: true });
    }
    hydrate(id, d) {
        return {
            id,
...
class FirestoreSessionStore {
    ...
    async resolve(sessionId) {
        ...
        if (new Date(s.expiresAt).getTime() <= Date.now()) {
            // Expired: delete lazily, treat as gone.
...
//# sourceMappingURL=filestoreStores.js.map