"use strict";
(() => {
  // client/vision-client.ts
  var VisionAgent = (() => {
    const CONFIG = {
      endpoint: "/api/vision",
      sampleIntervalMs: 1800,
      // base cadence while passively watching
      activeIntervalMs: 700,
      // faster cadence when a watch condition is active
      changeThreshold: 18,
      // mean abs grayscale diff (0-255) counted as "changed"
      activeChangeThreshold: 8,
      // more sensitive while actively watching
      diffSize: 32,
      // downscaled edge used for the cheap diff
      maxEdge: 640,
      // longest edge of the frame sent to the model
      jpegQ: 0.7,
      sceneMemory: 6,
      // observations retained for short-term context
      maxBackoffMs: 15e3
    ...
    const state = {
      ...
      frameIndex: 0,
      lastSig: null,
      // signature of the last SENT frame
      lastSpoken: "",
      observations: [],
      watchFor: null,
    ...
    function setTaskContext(task, mode) {
      if (task !== void 0) state.taskContext.task = task;
      if (mode !== void 0) state.taskContext.mode = mode;
    }
    ...
    function onObserveError(sig, message) {
      state.lastSig = sig;
    ...
    async function observe(question, sig) {
      ...
      try {
        const body = {
          image: grabFrame(),
          tier: question ? "deep" : "fast",
          // model selection; distinct from task_context.mode
          session_id: state.sessionId,
          task_context: state.taskContext,
          user_transcript: question || "",
          recent: state.observations.slice(-3).map((o) => o?.scene?.summary).filter((s) => !!s),
          media_meta: {
    ...
    function applyDirective(d, interrupt = false) {
      ...
      state.watchFor = d.watch_for ?? null;
