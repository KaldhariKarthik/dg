...
/**
 * Real-time progress events emitted during a turn. The streaming chat route
 * forwards these to the browser over SSE so the UI's trace flow shows what
 * ACTUALLY happened (which agent ran, why, when memory updated) instead of a
 * mock. Non-streaming callers pass no callback and pay nothing.
 */
export type OrchestratorEvent =
    | { type: "thinking"; agent: string }
    | { type: "route"; agent: string; reason: string }
    | { type: "agent"; agent: string; message: string }
    | { type: "synth" }
    | { type: "memory"; n: number }
    | { type: "done"; message: string };

export type OrchestratorEmit = (ev: OrchestratorEvent) => void;
...
export class Orchestrator {
    ...
    async run(
        ...
        while (steps < maxSteps) {
            fire({ type: "thinking", agent: "router" });
            ...
            if (decision.action === "finish") {
                ...
                if (soFar.length === 0) {
                    ...
                    fire({ type: "route", agent: fallbackAgent, reason: "router finished with no work — one safe hop" });
                    fire({ type: "thinking", agent: fallbackAgent });
                    const res = await agent.handle(req, ctx);
                    soFar.push(res);
                    fire({ type: "agent", agent: res.from, message: res.message });
            ...
            fire({
                ...
                reason: decision.reason ?? "best fit for this message",
            });
            fire({ type: "thinking", agent: decision.agent });
            const res = await agent.handle(req, ctx);
            soFar.push(res);
            fire({ type: "agent", agent: res.from, message: res.message });
        ...
        if (soFar.length > 1) fire({ type: "synth" });
        ...
        // chat replies.)
        await this.updateMemory(ctx, soFar, fire);

        fire({ type: "done", message: response.message });
        return response;
    }
    ...
    private async updateMemory(
        ...
        try {
            ...
            const n =
                Object.keys(delta.preferences).length +
                delta.past_patterns.length +
                delta.long_term_facts.length;
            emit({ type: "memory", n });
