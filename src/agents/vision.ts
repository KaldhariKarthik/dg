...
export class VisionAgent implements Agent {
    ...
    async handle(req: AgentRequest, ctx: Context): Promise<AgentResponse> {
        ...
        const activePlan = guided ? (ctx.state.activePlan as Plan | undefined) : undefined;
    ...
    private validateStepAction(
        ...
        if (index === null || !plan) return null;
        if (!Number.isInteger(index) || index < 0 || index >= plan.steps.length) return null;
        if (plan.steps[index].done) return null; // already done — nothing to do
        return index;
    }

    /** Plain-camera reasoning (unchanged behavior). */
    private describeSystem(): string {
        return (
    ...
    private guidedSystem(): string {
        return (
            "You are DaVinci's vision reasoner, GUIDING the user hands-on through a " +
